    ...
    def select_move(self):
        ...
        while True:
            if  self.is_god_spell_open == True :
                print("<=======    C H O O S E   T H E  S P E L L    =======>")
...
class Deck(object):
    ...
    def distribute(self, player1, player2):
        self.shuffle()
        print("<<----------->> C A R D   DISTRIBUTION ....in Progress  <<----------->>")
        self.player1 = player1
        self.player2 = player2

        print("Total no of cards: {}".format(len(self.cards)))
        lv_count = len(self.cards) / 2
        print("Each player gets {} cards".format(lv_count))
...
class Game(object):
    ...
    def compare(self, p1_card:0,p2_card:0):
        if self.player1.cards[p1_card].get_characterstic(self.sel_charcterstic) < self.player2.cards[p2_card].get_characterstic(self.sel_charcterstic):
            print("* * * * * * * * Congrats {} won the challenge * * * * * * * * ".format(self.player2.name))
            self.player2.points =  self.player2.points + 1
            self.player1.has_next_move = False
            self.player2.has_next_move = True

            print("* * * * * * * * Hey {} !... Its your turn * * * * * * * * ".format(self.player2.name))
        else:
            print("* * * * * * * * Congrats {} won the challenge * * * * * * * *".format(self.player1.name))
            self.player1.points = self.player1.points + 1
            self.player1.has_next_move = True
            self.player2.has_next_move = False

            print("* * * * * * * * Hey {} !... Its your turn * * * * * * * * ".format(self.player1.name))
    ...
    def select_resurrect(self):
        while True:
            print("Enter 'r' to proceed with Resurrcet and n to continue normall")
            move = input()
            if  (move != 'r' and move != 'n' ):
                print("Please enter a valid input")
            else:
                break
        return move
    ...
    def display_score(self):
        ...
        if self.player2.points > self.player1.points:
            print("########## Congrats {} won the game ########## ".format(self.player2.name))
        else:
            print("########## Congrats {} won the game ########## ".format(self.player1.name))
